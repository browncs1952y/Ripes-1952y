...
#include "cs1952y_idrr.h"
#include "cs1952y_rrex.h"
...
public:
  enum Stage { IF = 0, ID = 1, RR = 2, EX = 3, MEM = 4, WB = 5, STAGECOUNT };
  CS1952y6sCPU(const QStringList &extensions)
    ...
    // Hazards
    decode->rs1 >> hazard_unit->id_rs1_id;
    decode->rs2 >> hazard_unit->id_rs2_id;
    control->alu1_sel >> hazard_unit->id_rs1_sel;
    control->alu2_sel >> hazard_unit->id_rs2_sel;
    idrr_reg->rs1_id_out >> hazard_unit->rr_rs1_id;
    idrr_reg->rs2_id_out >> hazard_unit->rr_rs2_id;
    idrr_reg->alu1_sel_out >> hazard_unit->rr_rs1_sel;
    idrr_reg->alu2_sel_out >> hazard_unit->rr_rs2_sel;
    idrr_reg->rd_id_out >> hazard_unit->idrr_rd_id;
    rrex_reg->rd_id_out >> hazard_unit->rrex_rd_id;
    exmem_reg->rd_id_out >> hazard_unit->exmem_rd_id;
    idrr_reg->mem_op_out >> hazard_unit->idrr_mem_op;
    rrex_reg->mem_op_out >> hazard_unit->rrex_mem_op;
    exmem_reg->mem_op_out >> hazard_unit->exmem_mem_op;
    j_or_b->out >> hazard_unit->jump_or_branch;

    hazard_unit->ifid_clear >> ifid_reg->clear;
    hazard_unit->idrr_clear >> idrr_reg->clear;
    hazard_unit->rrex_clear >> rrex_reg->clear;
    hazard_unit->exmem_clear >> exmem_reg->clear;

    hazard_unit->if_enable >> pc_reg->enable;
    hazard_unit->if_enable >> ifid_reg->enable;
    hazard_unit->rrex_enable >> rrex_reg->enable;
    ...
    // ID/RR
    ifid_reg->pc_out >> idrr_reg->pc_in;
    ifid_reg->pc4_out >> idrr_reg->pc4_in;
    imm_sel->out >> idrr_reg->imm_in;
    decode->rd1 >> idrr_reg->rd_id_in;
    
    decode->rs1 >> idrr_reg->rs1_id_in;
    decode->rs2 >> idrr_reg->rs2_id_in;
    
    control->alu1_sel >> idrr_reg->alu1_sel_in;
    control->alu2_sel >> idrr_reg->alu2_sel_in;
    control->alu_ctrl >> idrr_reg->alu_op_in;
    control->pc_add1 >> idrr_reg->pc_add1_in;
    control->mem_w >> idrr_reg->mem_wr_in;
    control->mem_op >> idrr_reg->mem_op_in;
    control->jump >> idrr_reg->jump_in;
    control->branch >> idrr_reg->branch_in;
    control->inv_zero >> idrr_reg->inv_zero_in;
    control->rd_sel >> idrr_reg->rd_sel_in;
    control->reg_w >> idrr_reg->reg_wr_in;

    hazard_unit->idrr_enable >> idrr_reg->enable;
    ifid_reg->valid_out >> idrr_reg->valid_in;
    
    // ** REGISTER READ **
    
    // Read register file
    idrr_reg->rs1_id_out >> registers->r1_addr;
    idrr_reg->rs2_id_out >> registers->r2_addr;
    
    // RR/Ex
    idrr_reg->pc_out >> rrex_reg->pc_in;
    idrr_reg->pc4_out >> rrex_reg->pc4_in;
    registers->r1_out >> rrex_reg->reg1_in;
    registers->r2_out >> rrex_reg->reg2_in;
    idrr_reg->imm_out >> rrex_reg->imm_in;
    idrr_reg->rd_id_out >> rrex_reg->rd_id_in;
    
    idrr_reg->rs1_id_out >> rrex_reg->rs1_id_in;
    idrr_reg->rs2_id_out >> rrex_reg->rs2_id_in;
    
    idrr_reg->alu1_sel_out >> rrex_reg->alu1_sel_in;
    idrr_reg->alu2_sel_out >> rrex_reg->alu2_sel_in;
    idrr_reg->alu_op_out >> rrex_reg->alu_op_in;
    idrr_reg->pc_add1_out >> rrex_reg->pc_add1_in;
    idrr_reg->mem_wr_out >> rrex_reg->mem_wr_in;
    idrr_reg->mem_op_out >> rrex_reg->mem_op_in;
    idrr_reg->jump_out >> rrex_reg->jump_in;
    idrr_reg->branch_out >> rrex_reg->branch_in;
    idrr_reg->inv_zero_out >> rrex_reg->inv_zero_in;
    idrr_reg->rd_sel_out >> rrex_reg->rd_sel_in;
    idrr_reg->reg_wr_out >> rrex_reg->reg_wr_in;
    
    idrr_reg->valid_out >> rrex_reg->valid_in;
    ...
    rrex_reg->reg1_out >> fwd1_sel->get(FwdSel::NoFwd);
    ...
    rrex_reg->reg2_out >> fwd2_sel->get(FwdSel::NoFwd);
    forward->fwd2_sel >> fwd2_sel->select;
      
    fwd1_sel->out >> alu_op1_sel->get(ALU1Sel::REG1);
    rrex_reg->pc_out >> alu_op1_sel->get(ALU1Sel::PC);
    rrex_reg->alu1_sel_out >> alu_op1_sel->select;
    alu_op1_sel->out >> alu->op1;

    fwd2_sel->out >> alu_op2_sel->get(ALU2Sel::REG2);      
    rrex_reg->imm_out >> alu_op2_sel->get(ALU2Sel::IMM);
    rrex_reg->alu2_sel_out >> alu_op2_sel->select;
    alu_op2_sel->out >> alu->op2;
    
    rrex_reg->alu_op_out >> alu->ctrl;
    
    // J/B PC address
    rrex_reg->pc_add1_out >> pc_add1->select;
    rrex_reg->pc_out >> pc_add1->get(PCAdd1::PC);
    fwd1_sel->out >> pc_add1->get(PCAdd1::RS);
    pc_add1->out >> pc_add->op1;
    
    rrex_reg->imm_out >> pc_add->op2;
    
    // Ex/Mem
    rrex_reg->pc4_out >> exmem_reg->pc4_in;
    fwd2_sel->out >> exmem_reg->reg2_in;
    rrex_reg->imm_out >> exmem_reg->imm_in;
    rrex_reg->rd_id_out >> exmem_reg->rd_id_in;
    
    alu->res >> exmem_reg->alu_res_in;
    alu->zero >> exmem_reg->alu_zero_in;
    pc_add->out >> exmem_reg->jb_pc_in;
    
    rrex_reg->mem_wr_out >> exmem_reg->mem_wr_in;
    rrex_reg->mem_op_out >> exmem_reg->mem_op_in;
    rrex_reg->jump_out >> exmem_reg->jump_in;
    rrex_reg->branch_out >> exmem_reg->branch_in;
    rrex_reg->inv_zero_out >> exmem_reg->inv_zero_in;
    rrex_reg->rd_sel_out >> exmem_reg->rd_sel_in;
    rrex_reg->reg_wr_out >> exmem_reg->reg_wr_in;
    
    rrex_reg->valid_out >> exmem_reg->valid_in;
    ...
    // Exceptions
    decode->ecall >> idrr_reg->ecall_in;
    idrr_reg->ecall_out >> rrex_reg->ecall_in;
    rrex_reg->ecall_out >> exmem_reg->ecall_in;
  ...
  SUBCOMPONENT(idrr_reg, TYPE(CS1952yIDRR<XLEN>));
  SUBCOMPONENT(rrex_reg, TYPE(CS1952yRREx<XLEN>));
  ...
  unsigned int getPcForStage(StageIndex idx) const override {
        ...
        switch (idx.index()) {
            ...
            case RR: return idrr_reg->pc_out.uValue();
            case EX: return rrex_reg->pc_out.uValue();
            case MEM: 
            return exmem_reg->pc4_out.uValue() == 0 ? 0 : exmem_reg->pc4_out.uValue() - 4;
  ...
  StageInfo stageInfo(StageIndex stage) const override {
    ...
    case RR: stageValid &= idrr_reg->valid_out.uValue(); break;
    case EX: stageValid &= rrex_reg->valid_out.uValue(); break;
    ...
    case RR: stageValid &= isExecutableAddress(idrr_reg->pc_out.uValue()); break;
    case EX: stageValid &= isExecutableAddress(rrex_reg->pc_out.uValue()); break;
    ...
    case ID:
      ...
      break;
    case RR:
      if (m_cycleCount > RR & idrr_reg->valid_out.uValue() == 0) {
        state = StageInfo::State::Flushed;
      }
      break;
    case EX: {
      if (m_cycleCount > EX && rrex_reg->valid_out.uValue() == 0) {
        state = StageInfo::State::Flushed;
      }
      break;
    }
