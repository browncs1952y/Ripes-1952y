...
public:
CS1952y5sHazard(std::string name, SimComponent *parent) : Component(name, parent) {    
  
    idex_clear << [=] {
        return jump_or_branch.uValue() ||
            isLoadHazard(idex_mem_op.uValue(), idex_rd_id.uValue(), rs1_id.uValue(), rs1_sel.uValue(), rs2_id.uValue(), rs2_sel.uValue());
    };
    
    if_enable << [=] { return jump_or_branch.uValue() ||
        !isLoadHazard(idex_mem_op.uValue(), idex_rd_id.uValue(), rs1_id.uValue(), rs1_sel.uValue(), rs2_id.uValue(), rs2_sel.uValue()); };
  ...
  bool isLoadHazard(auto m_o, auto rd, auto rs1, auto rs1_s, auto rs2, auto rs2_s) {
    return isLoad(m_o) && rd != 0 && ((rs1_s == ALU1Sel::REG1 && rd == rs1) || (rs2_s == ALU2Sel::REG2 && rd == rs2));
  }
  
  bool isLoad(auto m_o) {
    return m_o == MemOp::LB || m_o == MemOp::LH || m_o == MemOp::LW || m_o == MemOp::LBU || m_o == MemOp::LHU;
  }
