...
/*
 * This class adds a "zero" output to the ALU component, which is high if the
 * ALU result is 0 and low otherwise
 */
...
constexpr int32_t div_overflow32_course = (-2147483648); //-2^(32-1)
constexpr int64_t div_overflow64_course = (LLONG_MIN);   //-2^(64-1)

template <unsigned XLEN>
class CS1952yALU : public Component {
public:
  SetGraphicsType(ALU);

  VSRTL_VT_U do_computation(const SimPort &ctrl, const SimPort &op1,
                            const SimPort &op2) {
    switch (ctrl.uValue()) {
    case ALUOp::ADD:
    ...
    case ALUOp::DIV: {
      const VSRTL_VT_S overflow =
          (ctrl.uValue() == ALUOp::DIVW) ||
              ...
              ? div_overflow32_course
              : div_overflow64_course;
      if (op2.sValue() == 0) {
        return VT_U(-1);
    ...
    case ALUOp::SLW:
      return VT_U(
          signextend<32>(op1.uValue() << (op2.uValue() & generateBitmask(5))));
  ...
  CS1952yALU(const std::string &name, SimComponent *parent)
      : Component(name, parent) {
    res << [=] { return do_computation(ctrl, op1, op2); };
    zero << [=] {
      const auto result = do_computation(ctrl, op1, op2);
      return result == 0 ? 1 : 0;
    };
  }
