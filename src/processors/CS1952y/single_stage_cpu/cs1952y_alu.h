...
/*
 * This class adds a "zero" output to the ALU component, which is high if the ALU result is 0 and low otherwise
 */
...
public:
  ...
  VSRTL_VT_U do_computation(const SimPort& ctrl, const SimPort& op1, const SimPort& op2) {
    switch (ctrl.uValue()) {
      case ALUOp::ADD:
  ...
  CS1952yALU(const std::string &name, SimComponent *parent) : Component(name, parent) {
