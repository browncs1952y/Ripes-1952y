...
public:
  CS1952y1sCPU(const QStringList &extensions)
    ...
    pc_add->out >> pc_sel->get(PCSel::JB);

    j_or_b->out >> pc_sel->select;
    control->jump >> *j_or_b->in[0];
    branch->out >> *j_or_b->in[1];
    control->branch >> *branch->in[0];
    ...
    pc_reg->out >> alu_op1_sel->get(ALU1Sel::PC);
    control->alu1_sel >> alu_op1_sel->select;
    // Select op2
