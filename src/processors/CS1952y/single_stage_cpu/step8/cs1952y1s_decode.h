...
public:
  ...
  CS1952y1sDecode(std::string name, SimComponent *parent) : Component(name, parent) {
    // Registers
    ...
    // Immediates
    imm_I << [=] { return instr.sValue() >> 20; }; // bits 20 to 31 (sign-extended)
    imm_Ishift << [=] { return (instr.uValue() >> 20) & 0x001f; }; // bits 20 to 24
    imm_U << [=] { return instr.sValue() & 0xfffff000; }; // bits 12 to 31
    imm_S << [=] { return ((instr.sValue() & 0xfe000000) >> 20) | ((instr.uValue() >> 7) & 0x001f); }; // bits 7 to 11 and 25 to 31
    imm_J << [=] {
                  return ((instr.sValue() >> 11) & 0xfff00000) | // bit 31 (sign-extended to 12 bits)
                          (instr.uValue() & 0x000ff000) | // bits 12 to 19
                          ((instr.uValue() & 0x00100000) >> 9) | // bit 20
                          ((instr.uValue() & 0x7fe00000) >> 20); // bits 21 to 30
                  }; // imm_J
    imm_B << [=] { return ((instr.sValue() >> 19) & 0xfffff000) | // bit 31 (sign-extended to 20 bits)
